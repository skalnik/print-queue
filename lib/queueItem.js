...
var QueueItem = (function () {
  ...
  QueueItem.prototype.fetchThumbnailUrl = function () {
    ...
    QueueItem.update(this.id,
      { thumbnail: this.thumbnailUrl() },
      function (err) {
        if (err) { console.log('Error saving thumbnail', err.message); }
      }
    );
  };

  QueueItem.prototype.thumbnailUrl = function () {
    ...
    if (matches = thingiverse.exec(this.url)) {
      ...
      request.get(url, function (err, res, body) {
        if (err) { return ''; }
        return JSON.parse(body).thumbnail;
      });
    }
  };
